...
@extract($_SESSION);
	...
	$regist_day = date("Y-m-d (H:i)");  // 현재의 '년-월-일-시-분'을 저장
		...
		$upfile_tmp_name = $_FILES["upfile"]["tmp_name"];    //임시 저장 파일
		$upfile_type     = $_FILES["upfile"]["type"];       //파일의 종류  jpg,png...
		$upfile_size     = $_FILES["upfile"]["size"];      //실용량(byte)
		$upfile_error    = $_FILES["upfile"]["error"];    //
		*/
/*용량의 크기
byte - kb - mb - gb - tb   (1024byte = 1kb)
*/
	// 다중 파일 업로드
	$files = $_FILES["upfile"];
	...
	for ($i=0; $i<$count; $i++)   //업로드 할 파일의 정보를 빼내는 코드
	{
		...
		if (!$upfile_error[$i])   //파일에 에러가 없으면
		{
				...
				($upfile_type[$i] != "image/pjpeg") )
			{
				...
				//move_uploaded_file(임시저장파일명, 업로드될파일경로)->파일 업로드하는 메소드
